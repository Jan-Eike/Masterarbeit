...
if __name__ == "__main__":
    ...
    if args.prediction_file is None:
        ...
        print((df_reference[keys]==df_predicted[keys])[(df_reference[keys]==df_predicted[keys])["Premise"] == False])
        print(df_reference.shape, df_predicted.shape)
        print(df_reference["Premise"].iloc[261])
        print(df_predicted["Premise"].iloc[261])
        print(df_reference.set_index(keys))
        print(df_predicted.set_index(keys))
        joined_df = df_reference.merge(right=df_predicted, how="outer", on=keys, indicator=True)
        print(joined_df[joined_df["_merge"] == "left_only"].iloc[:3])
        print(joined_df[joined_df["_merge"] == "right_only"].iloc[:3])
